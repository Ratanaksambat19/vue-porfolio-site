<template>
    <div>
    <svg v-if="name === 'clock'" class="Clock" xmlns="http://www.w3.org/2000/svg" width="229.575" height="188.911" viewBox="0 0 229.575 188.911">
        <g id="Group_2" data-name="Group 2" transform="translate(-900.535 -459.043)">
            <path id="Path_4" data-name="Path 4" d="M1093.45,602.806c11.391-6.167,18.242-13.967,20.526-22.1,3.308-11.778-2.975-24.256-18.958-33.484-13.612-7.86-31.668-11.859-49.8-11.965h.006l-38.79-.386c-.3,0-.594-.007-.893-.009h-.235c-17.784-.072-35.627,3.6-49.407,11.06-27.887,15.1-28.59,39.987-1.57,55.586,13.358,7.713,30.994,11.71,48.782,11.956h.213l.692.007,38.73.386h0C1060.953,614.119,1079.331,610.451,1093.45,602.806Z" transform="translate(15.278 34.086)" fill="#3f5052" opacity="0.35" style="mix-blend-mode: multiply;isolation: isolate"/>
            <g id="Group_1" data-name="Group 1" transform="translate(900.535 459.043)">
                <g class="clock-body">
                    <path id="Path_5" data-name="Path 5" d="M1040.924,559.515c0-16.992-4.874-35.483-13.1-52.832l-91.336,52.745c10.913,15.789,24.487,29.253,39.2,37.748,8.226,4.749,16.095,7.529,23.344,8.5l-3.708,11.262,26.049-14.95h-.006C1033.435,595.29,1040.924,580.575,1040.924,559.515Z" transform="translate(-884.369 -437.624)" fill="#f05555"/>
                    <path id="Path_6" data-name="Path 6" d="M986.059,467.948c-18.151-10.479-34.569-11.394-46.4-4.4l0,0L913.43,478.6l13.29-.9c-3.782,7.354-5.891,16.739-5.891,27.912,0,24.611,10.223,52.365,26.032,75.237L1038.2,528.1C1026.294,502.979,1007.37,480.252,986.059,467.948Z" transform="translate(-894.737 -459.043)" fill="#f05555"/>
                </g>
                <path id="OuterFrame" data-name="Path 7" d="M1031,591.264c0,41.6-29.2,58.461-65.232,37.661s-65.232-71.385-65.232-112.983,29.207-58.462,65.232-37.664S1031,549.664,1031,591.264Z" transform="translate(-900.535 -454.4)" fill="#faf9e1"/>
                <path id="Inner-Clock" data-name="Path 8" d="M1013.906,578.507c0,33.28-23.363,46.768-52.186,30.127s-52.186-57.107-52.186-90.385,23.365-46.77,52.186-30.131S1013.906,545.227,1013.906,578.507Z" transform="translate(-896.489 -449.175)" fill="#f05555"/>
                <path id="Shadow-clock" data-name="Path 9" d="M970.418,603.114c-28.821-16.638-52.186-57.107-52.186-90.385,0-14.016,4.146-24.521,11.1-30.745-12.059,4.078-19.794,16.67-19.794,35.818,0,33.279,23.365,73.747,52.186,90.385,16.685,9.633,31.539,9.169,41.093.615C993.912,611.812,982.659,610.181,970.418,603.114Z" transform="translate(-896.489 -448.728)" fill="#b1b1b3"/>
                <path id="Tick-tock" data-name="Path 10" d="M975.819,546.357,956.3,535.077V503.92a3.116,3.116,0,0,0-1.409-2.44l-7.947-4.588c-.777-.449-1.409-.086-1.409.813v41.953a3.118,3.118,0,0,0,1.409,2.44l.023.012h0l28.851,16.675c.778.449,1.409-9.09,1.409-9.989h0A3.112,3.112,0,0,0,975.819,546.357Z" transform="matrix(0.995, -0.105, 0.105, 0.995, -930.201, -338.729)" fill="#526a70"/>
            </g>
        </g>
    </svg>

    <svg v-if="name === 'star'" class="Star" data-name="Group 6" xmlns="http://www.w3.org/2000/svg" width="251.379" height="214" viewBox="0 0 251.379 214">
        <path id="star-shadow" data-name="Path 17" d="M525.69,797.35c-1.917-2.045-1.576-4.542.88-6.437l.391-.3,27.166-20.955.219-.169L538.9,744.815a3.623,3.623,0,0,1-.546-2.623,5.736,5.736,0,0,1,3.072-3.691,16.123,16.123,0,0,1,7.527-1.883v0l24.719-.246h0a18.859,18.859,0,0,1,3.5.276l27.547,4.84,29.925-12.968a17.369,17.369,0,0,1,6.734-1.294h0l24.042-.24c.187-.005.373-.005.56-.005h.117a18.1,18.1,0,0,1,4.565.6c3.712,1.005,6.053,3.111,6.041,5.436l-.21,26.27,58.861,3.063c3.409,1.294,5.181,3.574,4.565,5.875a6.009,6.009,0,0,1-3.012,3.405,14.286,14.286,0,0,1-4.454,1.573l-65.029,19.989-13.573,25.268A6.8,6.8,0,0,1,651.174,821a15.235,15.235,0,0,1-5.983,1.792,19.278,19.278,0,0,1-2.2.1h0l-23.737.237h0c-3.619.109-7.111-.8-9.258-2.452l-27.112-20.833-22.745.465a19.211,19.211,0,0,1-3.131-.192l-21.411.438C531.445,800.633,527.6,799.391,525.69,797.35Z" transform="translate(-524.458 -609.131)" fill="#3f5052" opacity="0.35" style="mix-blend-mode: multiply;isolation: isolate"/>
        <g id='actual-star'>
            <path id="Path_18" data-name="Path 18" d="M519.776,739.977c-13.873-12.963-14.212-32.93-13.553-52.3L544.746,671.8a6.438,6.438,0,0,1,5.888.057h0l16.244,9.324L549.417,690Z" transform="translate(-318.231 -653.544)" fill="#ff8484"/>
            <path id="Path_19" data-name="Path 19" d="M573.216,684.982l-27.724-16.007a20.231,20.231,0,0,1,6.38-6.236c2.7-1.56,5.231-1.809,7.1-.82v0l16.244,9.322-7.344,4.527Z" transform="translate(-397.405 -661.331)" fill="#ff8484"/>
            <path id="Path_20" data-name="Path 20" d="M571.015,700.007l79.756,46.049,2.853,17.284c.659,3.969-.917,9.284-4.055,13.69a19.851,19.851,0,0,1-6,5.685,9.306,9.306,0,0,1-4.748,1.407l-20-.411-90.31-52.142Z" transform="translate(-445.479 -630.577)" fill="#ff8484"/>
            <path id="Path_21" data-name="Path 21" d="M530.185,727.374l.345-.255,19.434-56.012a24.236,24.236,0,0,1,2.772-5.518L580.46,681.6l7.249,12.51,4.931-2.032c-.659,19.373-.32,39.341,13.553,52.3l-.817,1.377,4.565,27.669Z" transform="translate(-404.648 -657.944)" fill="#f05555"/>
            <path id="Path_22" data-name="Path 22" d="M548.481,730.7a23.466,23.466,0,0,1,8.24-12.661l.609-.454,90.31,52.142-18.878-.39-31.033,36.475-3.991,23.109-15.6-8.949v0c-2.463-1.219-3.635-4.443-3.05-8.635l7.421-53.52-31.447-16.709C548.282,739.628,547.282,735.594,548.481,730.7Z" transform="translate(-474.295 -616.597)" fill="#ff8484"/>
            <path id="Path_23" data-name="Path 23" d="M505.012,780.6a23.49,23.49,0,0,1,8.24-12.663l43.455-32.271,19.436-56.012c1.72-4.958,5.272-9.516,9.152-11.755s7.427-1.783,9.146,1.194l19.445,33.56,43.455-17.9c3.847-1.582,7.035-.357,8.236,3.149s.2,8.692-2.58,13.386L631.554,754.3l7.419,44.952c.659,3.969-.917,9.284-4.057,13.69a19.829,19.829,0,0,1-6,5.685,9.3,9.3,0,0,1-4.748,1.409l-38.874-.8-38.872,45.685A18.566,18.566,0,0,1,541.671,869a8.226,8.226,0,0,1-5.994,1.24c-3.145-.776-4.716-4.278-4.057-9.006l7.423-53.519L507.594,791C504.813,789.517,503.813,785.484,505.012,780.6Z" transform="translate(-414.698 -657.232)" fill="#f05555"/>
        </g>
    </svg>

    <svg v-if="name === 'target'" class="Target" xmlns="http://www.w3.org/2000/svg" width="244.554" height="212.192" viewBox="0 0 244.554 212.192">
        <g id="Group_25" data-name="Group 25" transform="translate(-702.517 -848.613)">
            <path id="Path_11" data-name="Path 11" d="M898.8,998.452c13.373-7.241,21.419-16.4,24.1-25.95,3.882-13.828-3.492-28.477-22.253-39.311-15.985-9.227-37.184-13.923-58.47-14.049h.005l-22.744-.182h-.17l-.279,0,0,0c-21-.138-42.09,4.173-58.363,12.985-32.74,17.726-33.565,46.944-1.843,65.259.3.172.611.334.914.5,15.538,8.7,35.795,13.223,56.232,13.528v0l.56.005c.4,0,.812.008,1.217.008l21.568.175-.005,0C860.654,1011.731,882.231,1007.425,898.8,998.452Z" transform="translate(23.166 49.369)" fill="#3f5052" opacity="0.35" style="mix-blend-mode: multiply;isolation: isolate"/>
            <g id="Group_4" data-name="Group 4" transform="translate(702.517 848.613)">
            <path id="Path_12" data-name="Path 12" d="M788.062,859.069c-21.31-12.3-40.585-13.376-54.468-5.17l0-.005-15.187,8.839,3.159,2.481c-6.418,9.229-10.087,22.1-10.087,38.07,0,28.894,12,61.476,30.562,88.329l107.23-61.923C835.3,900.2,813.083,873.514,788.062,859.069Z" transform="translate(-696.226 -848.613)" fill="#ff8484"/>
            <path id="Path_13" data-name="Path 13" d="M852.042,958.279c0-19.949-5.722-41.658-15.373-62.025l-107.23,61.923c12.811,18.536,28.747,34.343,46.019,44.316,12.638,7.3,24.561,10.643,35.068,10.448l3.276,4.1,15.289-8.891-.005,0C843.253,1000.279,852.042,983,852.042,958.279Z" transform="translate(-683.622 -815.177)" fill="#ce3e3d"/>
            <path id="Path_14" data-name="Path 14" d="M855.685,996.935c0-48.377-33.651-107.085-75.4-131.924h0c-.4-.235-.79-.49-1.19-.72-42.3-24.422-76.583-4.624-76.583,44.214S736.8,1016.731,779.1,1041.151c.4.231.793.429,1.19.652h0C822.034,1065.169,855.685,1045.314,855.685,996.935Z" transform="translate(-702.517 -844.951)" fill="#f05555"/>
            <g id="Group_3" data-name="Group 3" transform="translate(19.911 34.591)">
                <path id="big-circle" data-name="Path 15" d="M770.888,890.828c24.491,14.139,44.418,48.656,44.418,76.935s-19.927,39.786-44.418,25.643-44.418-48.656-44.418-76.934,19.927-39.786,44.418-25.645Zm0-14.149c-31.3-18.07-56.672-3.421-56.672,32.72s25.373,80.087,56.672,98.157,56.671,3.421,56.671-32.718-25.373-80.089-56.671-98.159Z" transform="translate(-714.217 -868.939)" fill="#faf9e1"/>
                <path id="small-circle" data-name="Path 16" d="M757.624,910.839c6.756,3.9,12.255,13.426,12.255,21.225s-5.5,10.975-12.255,7.074-12.253-13.422-12.253-21.224,5.5-10.977,12.253-7.076Zm0-14.147c-13.533-7.813-24.507-1.481-24.507,14.147s10.974,34.634,24.507,42.447,24.506,1.481,24.506-14.147-10.973-34.634-24.506-42.447Z" transform="translate(-700.952 -851.81)" fill="#faf9e1"/>
            </g>
            </g>
        </g>
    </svg>

    </div>
</template>

<script>
export default {
    props: ['name']
}
</script>

<style lang="scss" scoped>


    .Target:hover #small-circle {
        animation-duration: 1s;
        animation-delay: 0.3s;
        animation-direction: alternate;
        animation-iteration-count: infinite;
        animation-name: animateTarget;
      @keyframes animateTarget {
        from{
            opacity: 1;
        }
        to {
            opacity: 0.6;
            transform: translate(-720.952px, -851.81px);
            }
        }   
    }

     .Star:hover #actual-star {
        animation-duration: .8s;
        animation-delay: 0.3s;
        animation-direction: alternate;
        animation-iteration-count: infinite;
        animation-name: animateStar;
        
      @keyframes animateStar {
        from{
            opacity: 1;
        }
        to {
            opacity: 0.5;
            transform: scale3d(1);
            transform: translateY(10%);
            transition: cubic-bezier(0.075, 0.82, 0.165, 1);
            }
        }
     }



     .Clock:hover #Inner-Clock{
        animation-duration: 3s;
        animation-delay: 0.4s;
        animation-direction: alternate-reverse;
        animation-iteration-count: infinite;
        animation-name: animateClock;
        

        @keyframes animateClock {
            from {
                opacity: 1;
                transform: translate(-896.489px, -449.175px);
            }

            to {
                transform-origin: 40% -50%;
                transform: translate(-194.737px, -459.043px);
            }
        }
     }
    
    .Clock:hover #OuterFrame{
        animation-duration: 3.5s;
        animation-delay: 0.4s;
        animation-direction: alternate-reverse;
        animation-iteration-count: infinite;
        animation-name: animateClock;
        

        @keyframes animateClock {
            from {
                opacity: 1;
                transform: matrix(0.895, -0.105, 0.105, 0.995, -830.201, -535.729);
            }

            to {
                transform-origin: 40% -30%;
            }
        }
     }

      .Clock:hover #Shadow-clock{
        animation-duration: 3s;
        animation-delay: 0.4s;
        animation-direction: alternate-reverse;
        animation-iteration-count: infinite;
        animation-name: animateClock;
        

        @keyframes animateClock {
            from {
                opacity: 1;
                transform: matrix(0.810, -0.105, 0.105, 0.995, -770.201, -360.729);
            }

            to {
                transform-origin: 40% -50%;
            }
        }
     }

      .Clock:hover #Tick-tock{
        animation-duration: 3s;
        animation-delay: 0.4s;
        animation-direction: alternate-reverse;
        animation-iteration-count: infinite;
        animation-name: animateTick;
        

        @keyframes animateTick {
            from {
                opacity: 1;
            }

            to {
                opacity: 0.8;
                transform: matrix(0.970, -0.105, 0.105, 0.995, -930.201, -338.729);
            }
        }
     }

</style>